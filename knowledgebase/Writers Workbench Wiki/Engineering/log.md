...
## [2026-06-20] build | Engine E2E-1 — library.email-content shipped

Implemented the on-demand "email me X" op (CR-010 A2 / [[engine-e2e-parity-sprints]] E2E-1): `library.email-content` in library_step (inline-content mode + DB-resolve for outline/short_story/chapter/newsletter/research/blog + graceful not-found), catalog entry, Gemini routing rule + deterministic `_email_params` heuristic branch, and a shared dependency-free `markdown_to_html` helper. Added 4 hub-router routing tests + 4 op unit tests (43 router-suite green; gateway+smoke 32 green; ruff clean). Flipped the 14 `impl: pending` email-content tags in [[engine-chat-e2e-suite]] to `built (E2E-1)`. Live DEV run still validates real Postal send.

## [2026-06-20] build | Engine E2E-2 — versions + revert + lifecycle trash shipped

Added `library.versions` (outline/content list+get) and `library.revert` (outline+chapter, snapshot-before-overwrite, no-mutation-on-error) to library_step; extended `library.lifecycle` with delete (soft-delete + content_versions snapshot + published-delete guard), undelete, and list_deleted (+content_type filter), plus delete/undelete email verbs. delete/undelete forced async via a build_dispatch_plan per-action override (E2E-2 R94/R98). Catalog entries + Gemini rule + deterministic heuristic branches (revert before version-history so "revert … to version N" rolls back). 13 new tests (4 routing + 9 op); flipped the 10 versions/revert `impl: pending` tags to built (E2E-2) in [[engine-chat-e2e-suite]]. router+gateway+smoke green; ruff clean.

## [2026-06-20] build | Engine E2E-3 — chapter.newsletter (topic newsletter as hub op)

Added `chapter.newsletter` (topic newsletter mirroring chapter.blog: Perplexity research → genre-toned subject_line/pre_header/intro/sections/outro → published_content_v2 content_type=newsletter → CR-009 email). Chosen over bridging the F2 curated saga because the R07/R16/R44 assertions describe the n8n topic newsletter (published_content_v2), not the saga (newsletter_sends). Catalog + heuristic branch + Gemini rule + `_newsletter_params`. Fixed a latent extract_json import bug shared with `_op_blog`. 4 new tests; flipped R07/R16/R44/V05 to built (E2E-3); V31 stays pending (callback + multi-task). 91 engine tests green; ruff clean.

## [2026-06-20] build | Engine E2E-4 — chapter.plan dual-arc depth + arc fidelity (structural)

Audit found the linchpin: nothing loaded story_arcs_v2.prompt_text (arcs were name-only). Built writer_engine.story_arcs.load_story_arc; expanded SubChapterBrief (number/title/brief/arc_beat/characters/setting/emotional_tone/connects_to_book_arc); chapter.plan dual-arc (book arc + per-chapter "using <arc>" override loaded + injected + prev/next context + rich sub_chapters + persisted into outline.chapters[N].chapter_outline w/ snapshot); brainstorm.story arc beats + per-chapter arc_notes; chapter.write auto-loads saved chapter outline + opt-in auto-plan guard (R116); routing extracts chapter_story_arc/story_arc/title/chapter-count. 9 offline tests; full engine suite 320 green. Flipped R110-R116/R118/R120/R121 to built (E2E-4); R119 + R54-R69 arc-beat wording remain live-validated.

## [2026-06-20] build | Engine E2E-5 — notify.eve-callback (gated KB + outbound call)

New writer_engine.eve runs the n8n WF-16 Eve flow (KB cleanup → upload → attach → review/brainstorm first_message → outbound call → reset), every real ElevenLabs call GATED behind ELEVENLABS_API_KEY+AGENT_ID (unset → dry run, PROD Eve agent untouched per baseline protection). notify._op_eve_callback resolves content (published_content_v2/research_reports_v2) and skips the callback on not-found (V30). Routing: "pull up X and call me back" + "get my X and help me improve" → notify.eve-callback (catalog + Gemini rule + heuristic). 6 offline tests; full engine suite 324 green. Flipped V26-V30 to built (E2E-5, gated dry-run); V31 (multi-task) stays pending. ALL FIVE E2E sprints (E2E-1..E2E-5) now shipped.

## [2026-06-21] build | Engine E2E-3/4/5 — closed the remaining gaps (V31, R119, R54-R61, V30)

User required E2E-3/4/5 fully satisfied. Built: (R54-R61) brainstorm.short-story now loads the named arc beats from story_arcs_v2, honours the section count (Freytag=5 not 3), and emits per-beat arc_notes; routing extracts story_arc/title/sections. (R119) chapter.qa loads the chapter prose + roster and runs a deterministic character-name cross-check incl. rename detection (Maya≠Maria via edit distance); "check ... character name consistency" routes to chapter.qa; quoted-title extraction fixed. (V30) eve-callback now does a genuine SYNC resolve in the gateway — not-found returns kind=data with NO job_id and NO callback. (V31) multi-task fan-out: split_tasks + gateway _execute_multi route one "X and also Y" message to several jobs (HubResponse.jobs); newsletter + eve-callback both invoked. 12 new tests (router + gateway + op); full engine suite 333 green; ruff clean. ALL E2E suite test tags now built (only the generic tag-convention prose mentions pending). PROD Eve agent still untouched (gated).
