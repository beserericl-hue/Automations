...
## [2026-06-02] build | Engine test on 'The Burial Mound' → Workbench-Test-Output
Ran the craft engine directly on the real project 'The Burial Mound' (ancient-history, 63-ch outline) via `/internal/write/{tool}` and wrote artifacts to `Workbench-Test-Output/` so output can be read in the vault before UI cutover. Generated research (14 craft-categorized questions + Sal-and-the-potatoes report, sonar-pro, 298s) and chapters 0/1/2 (3418/2714/2621 words; 114/265/235s). Compare vs the 3 existing n8n UI chapters (6633/8070/7990 words). Headline: engine single-call chapters ≈ ⅓ the n8n length → needs the sub-chapter fan-out (F1-1). Brainstorm (fresh 63-ch outline) twice hit Railway's ~300s edge limit (502) → confirms long write tools must run async. 4 engine bugs found + fixed live: research category casing + chapter-QA double-envelope + draft-loss-on-QA-failure (PR #111), retired Perplexity model (PR #112). Per-stage times recorded as the chapter-optimization-sprint baseline. See [[Workbench-Test-Output/00-test-plan]].
