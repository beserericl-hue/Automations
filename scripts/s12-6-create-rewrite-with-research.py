...
const chUrl =
  ...
  '&deleted_at=is.null&select=id,title,content,summary,word_count,metadata&limit=1';
...
// 2. Update published_content_v2.content in place.
const pcUrl =
  supabaseUrl +
    ...
    nodes = [
        ...
        # Claude chain call with the credibility-first prompt.
        {
            "parameters": {
                "messages": {
                    "messageValues": [
                        {"role": "system", "message": "={{ $json.rewrite_system_prompt }}"},
                        {"role": "user", "message": "={{ $json.rewrite_user_prompt }}"},
                    ]
                },
                "batching": {},
            },
