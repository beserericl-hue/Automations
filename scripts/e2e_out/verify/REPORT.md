...
**Run:** 2026-07-01/02 · all 157 suite tests driven through the engine **chat API** (`/internal/hub`)
and **voice webhook** (`/internal/hub/voice`, the simulated-Eve surface), each polled to completion and
verified against the **DEV Supabase database** + completion **emails** (read from `eric@agileadtesting.com`).

## Result: 151 / 157 PASS · 6 FAIL — and all 6 FAILs are non-engine-bugs (2 are actual passes)

The 20 gaps (G1–G20) from the prior report are **all repaired and confirmed live**. The engine now
...
- **G1 — completion emails carry the deliverable** *(dominant prior failure, user-confirmed 3×)*: every
  ...
  - Outline email = full structured outline (premise + 5 characters + 10 numbered chapters incl.
    Prologue/Epilogue). Research email = full report. Blog/newsletter/short-story/chapter = full text.
    **Cover-art email embeds the image URL** (`…/cover-images/…/untitled.png`). ✅
- **G6 — word-count crash**: R06/R08/R14/R15/R38/R43/R45/R50/R52 blogs + short stories persist with real
  word counts (1293–2442). No more `int('1500 words')`. ✅
...
- **G12 — brainstorm persists**: R30 "The Seed Vault" → **8 chapters** (the requested count; was 39),
  `persisted=true`; R48/R110 persist; revise-outline loads + snapshots. ✅
...
- **G2/G4/G5/G20 — config table / genre / markdown / outline render**: `app_config_v2` (per-user),
  genre_slug preserved, indented headings, outline chapter numbers+beats. ✅
...
4. Search keywords included the verb "find" → matched polluted titles; verbs dropped.
5. Routing: lead-verb retrieval ("retrieve the outline for X", "find my draft story") read instead of
   regenerating; yields to version/trash/revert/callback.
