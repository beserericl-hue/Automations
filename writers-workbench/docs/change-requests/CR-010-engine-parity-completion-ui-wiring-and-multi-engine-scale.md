...
|---|---|---|---|
| **Format Kindle book** (`19_tool_format_kindle_book_v2`) | ❌ stub | `chapter_step/main.py:1462` `_op_format_kindle` returns `{"docx_storage_path": "kindle/<id>.docx"}` — no document is built | Either (a) implement real manuscript build in the engine (compile chapters → styled .docx/.epub → Supabase Storage → email link), or (b) route the hub `chapter.format-kindle` op to the **already-working** server `POST /api/export/docx` (uses the `docx` lib). **(b) is cheaper and reuses tested code** — recommended. n8n's Google-Docs flavor (page breaks, public share link) is optional polish. |
