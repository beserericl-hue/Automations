      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
          <EmptyState
            ...
            description="Ask Eve or use the chat to research a topic. Research reports will appear here."
          />
        </div>
      ) : (
        <>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {paginatedReports.map((report) => (
                  <tr key={report.id} onClick={() => navigate(`/research/${report.id}`)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/research/${report.id}`); } }} tabIndex={0} role="button" aria-label={`Open research report ${report.topic}`} className="hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-inset">
                    <td className="px-6 py-3 text-sm font-medium text-brand-700 hover:underline dark:text-brand-300">{report.topic}</td>
